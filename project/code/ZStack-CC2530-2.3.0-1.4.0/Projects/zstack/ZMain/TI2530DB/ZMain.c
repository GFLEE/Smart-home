...
int main( void )
{
  ...
  HAL_BOARD_INIT();    //初始化系统

  // Make sure supply voltage is high enough to run
  zmain_vdd_check(); //检查系统是否正常

  // Initialize board I/O
  InitBoard( OB_COLD );  //初始化IO

  // Initialze HAL drivers
  HalDriverInit();  //初始化硬件设备

  // Initialize NV System
  osal_nv_init( NULL );  //初始化系统模块

  // Initialize the MAC
  ZMacInit();   //初始化mac层，保证设备与协调器同步工作和休眠

  // Determine the extended address
  zmain_ext_addr();   //确定通信协议标准802.11 64位地址
  ...
  osal_start_system(); // 初始化应用层系统 No Return from here
